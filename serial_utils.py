...
def FindTtyByDriver(driver_name,
    ...
    for candidate in glob.glob('/dev/tty*'):
        ...
        if re.search(driver_name + '$', driver_path):
            if (interface_protocol is None or interface_protocol
                    == DeviceInterfaceProtocol(device_path)):
                if multiple_ports:
...
def FindTtyListByUsbVidPid(usb_vid, usb_pid):
    ...
    # now. Add a handler here to avoid chameleon crash until we get new image
    # of chameleon. (crbug.com/951703)
    try:
        import pyudev
    except ImportError:
        logging.error("Failed to import pyudev")
...
class SerialDevice(object):
    ...
    def __init__(self,
                 send_receive_interval_secs=0.2,
    ...
    def Connect(self,
        ...
        Args:
            driver: driver name of the target serial connection. used to look up port if port is not specified.
            usb_vid: USB VID of the target serial connection. used to look up port if port is not specified.
            usb_pid: USB PID of the target serial connection. used to look up port if port is not specified.
        ...
        if driver and not port:
            if usb_vid and usb_pid:
                if known_device_set:
                    ...
                    port = FindTtyByUsbVidPid(usb_vid,
                                              usb_pid,
                                              driver_name=driver)
            else:
        ...
        self.port = port
        self._serial = OpenSerial(port=port,
                                  baudrate=baudrate,
                                  bytesize=bytesize,
                                  parity=parity,
                                  stopbits=stopbits,
    ...
    def Send(self, command, flush=True):
        """Sends a command. It blocks at most write_timeout seconds.
        Args:
        ...
        try:
            ...
            if self.log:
                ...
                logging.info('Successfully sent %r. Took %.3f seconds', command, duration)
    ...
    def Receive(self, size=1):
        """Receives N bytes. It blocks at most timeout seconds.
        Args:
            size: number of bytes to receive. 0 means receiving what already in the input buffer.
        ...
        if len(response) == size:
            if self.log:
                duration = time.time() - start_time
                logging.info('Successfully received %r. Took %.3f seconds', response, duration)
            return response
        else:
            error_message = 'Receive %d bytes timeout after %.2f seconds' % (size, self._serial.getTimeout())
            if self.log:
                logging.warning(error_message)
            raise serial.SerialTimeoutException(error_message)

    def ReceiveLine(self):
        """Receives one line. It blocks at most timeout seconds.
    ...
    def SendReceive(self, command, size=1, retry=0, interval_secs=None, suppress_log=False):
        """Sends a command and returns a N bytes response.
        Args:
            command: command to send
            size: number of bytes to receive. 0 means receiving what already in the input buffer.
            retry: number of retry.
            interval_secs: #seconds to wait between send and receive. If specified, overrides self.send_receive_interval_secs.
        ...
        error_message = 'Timeout receiving %d bytes for command %r' % (size, command)

        if not suppress_log and self.log:
            logging.warning(error_message)
        raise serial.SerialTimeoutException(error_message)

    def SendExpectReceive(self, command, expect_response, retry=0, interval_secs=None):
        ...
        if self.log:
            if response == expect_response:
                logging.info(
                    'Successfully sent %r and received expected response %r',
